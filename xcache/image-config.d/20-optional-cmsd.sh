...
[program:xcache-cmsd]
command=/usr/bin/cmsd -c /etc/xrootd/xrootd-%(ENV_XC_IMAGE_NAME)s.cfg -k fifo -n %(ENV_XC_IMAGE_NAME)s -k %(ENV_XC_NUM_LOGROTATE)s -s /var/run/xrootd/cmsd-%(ENV_XC_IMAGE_NAME)s.pid -l /var/log/xrootd/cmsd.log
user=xrootd
autorestart=true
